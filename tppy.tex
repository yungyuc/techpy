...
下載後即可安裝 Anaconda。
或者你也可以打開 Ubuntu 的終端機 (terminal；提供命令行介面)，
輸入以下的指令，即可安裝 Anaconda (此處假設你使用的是一般 x86\_64 架構的 PC)：

\begin{minted}{bash}
$ mkdir -p ~/tmp
$ curl -sSL -o ~/tmp/anaconda.sh \
https://repo.continuum.io/archive/Anaconda3-4.3.0-Linux-x86_64.sh
$ bash ~/tmp/anaconda.sh -b -p $HOME/anaconda
\end{minted}
%stopzone

我們把 Anaconda 安裝到 \verb+~/anaconda+ 目錄下。
要使用所安裝的 Python，需要調整執行檔搜尋路徑。
請在 \verb+~/.bashrc+ 的尾巴加上一行：
...
因為系統中可能有其它的 Python 2 安裝，
所以我們在執行 Python 直譯器的時候，儘可能用 \verb+python3+ 指明使用 Python 3，
以免錯誤地執行了 Python 2。
